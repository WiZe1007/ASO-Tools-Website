...
class SeparateAccountTests(unittest.TestCase):
    ...
    def test_only_versioned_public_assets_get_immutable_caching(self):
        for client in self.clients:
            ...
            partial = client.get(path, headers={"Range": "bytes=0-99"})
            self.assertEqual(partial.status_code, 206)
            self.assertEqual(partial.data, response.data[:100])
            self.assertTrue(partial.cache_control.immutable)
            response.close()
            unchanged.close()
            partial.close()
            with client.get("/static/js/background-video.js") as unversioned:
                self.assertFalse(unversioned.cache_control.immutable)
            self.assertFalse(client.get("/static/missing.js?v=1").cache_control.immutable)
            self.login(client, self.admin)
            with patch.object(database_app, "build_store") as build:
                build.return_value.load_all_apps.return_value = []
                for private in ("/?v=1", "/admin/users?v=1", "/api/apps?v=1"):
